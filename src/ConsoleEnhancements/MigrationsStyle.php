...
class MigrationsStyle extends OutputStyle
{
	...
	public function title($message)
	{
		$message = '  ' . $message . '  ';
		$this->autoPrependBlock();
		$this->writeln($this->createBlock(
			$messages, $type, $style, $prefix, $padding, true));
		$this->newLine();
		$this->block(
			str_pad($message, min($this->getTerminalWidth(), static::MAX_LINE_LENGTH) - 2, '=', STR_PAD_BOTH),
			null, 'fg=blue', ' ', false);
	}
	...
	public function table(array $headers, array $rows)
	{
		...
		$style->setHorizontalBorderChar('<muted>-</muted>');
